...
export default function BackgroundSpline() {
  ...
  const parallaxY = useTransform(smooth, [0, 1], [0, -60]);
  const parallaxOpacity = useTransform(smooth, [0, 0.2, 1], [1, 0.95, 0.85]);
  ...
  useMotionValueEvent(smooth, 'change', (progress) => {
    ...
    const angle = progress * Math.PI * 2 * FULL_REVOLUTIONS;

    for (const { object, rotation } of snapshots) {
      object.rotation.x = rotation.x + angle;
    }
  });
